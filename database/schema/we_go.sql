...
CREATE TABLE `user` (
  ...
  -- Trust Score System columns
  `trust_score` INT NOT NULL DEFAULT 100 COMMENT 'Điểm tín nhiệm (0-100)',
  `is_verified` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Tài khoản đã xác thực đầy đủ',
  `verified_phone` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Đã xác thực số điện thoại',
  `verified_id` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Đã xác thực CCCD/CMND',
  `verification_docs` JSON DEFAULT NULL COMMENT 'Thông tin tài liệu xác thực',
  `last_score_update` DATETIME DEFAULT NULL COMMENT 'Lần cập nhật điểm gần nhất',
  ...
  KEY `ix_user_trust_score` (`trust_score`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `support_ticket` (
  ...
  `user_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'NULL = guest user',
  `guest_name` VARCHAR(150) DEFAULT NULL COMMENT 'Tên khách vãng lai',
  `guest_email` VARCHAR(190) DEFAULT NULL COMMENT 'Email khách vãng lai',
  `guest_phone` VARCHAR(30) DEFAULT NULL COMMENT 'SĐT khách vãng lai',
  `title` VARCHAR(255) NOT NULL,
  ...
  KEY `ix_ticket_guest_email` (`guest_email`),
...
-- Lịch sử thay đổi điểm tín nhiệm
CREATE TABLE `user_score_history` (
  `history_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_id` BIGINT UNSIGNED NOT NULL,
  `score_change` INT NOT NULL COMMENT 'Điểm thay đổi (+/-)',
  `old_score` INT NOT NULL COMMENT 'Điểm cũ',
  `new_score` INT NOT NULL COMMENT 'Điểm mới',
  `reason` VARCHAR(500) NOT NULL COMMENT 'Lý do thay đổi',
  `related_type` VARCHAR(50) DEFAULT NULL COMMENT 'Loại liên quan: booking, review, listing, verification',
  `related_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'ID của đối tượng liên quan',
  `admin_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'ID admin nếu thay đổi thủ công',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `score_config` (
  ...
  UNIQUE KEY `uq_action_type` (`action_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `user_badges` (
  ...
  `user_id` BIGINT UNSIGNED NOT NULL,
  `badge_type` VARCHAR(50) NOT NULL COMMENT 'Loại huy hiệu: verified, superhost, trusted, newcomer',
  `earned_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `expires_at` DATETIME DEFAULT NULL COMMENT 'Ngày hết hạn (NULL = vĩnh viễn)',
  PRIMARY KEY (`badge_id`),
  KEY `ix_badge_user` (`user_id`),
  KEY `ix_badge_type` (`badge_type`),
  CONSTRAINT `fk_badge_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Insert default score configurations
INSERT INTO `score_config` (`action_type`, `score_change`, `description`, `is_active`) VALUES
-- Positive actions
('verify_email', 5, 'Xác thực email thành công', 1),
('verify_phone', 5, 'Xác thực số điện thoại', 1),
('verify_id', 10, 'Xác thực CCCD/CMND', 1),
('first_booking', 5, 'Hoàn thành booking đầu tiên', 1),
('complete_booking', 1, 'Hoàn thành một booking', 1),
('receive_5_star', 2, 'Nhận đánh giá 5 sao', 1),
('receive_4_star', 1, 'Nhận đánh giá 4 sao', 1),
('account_6_months', 10, 'Tài khoản hoạt động > 6 tháng', 1),
('account_1_year', 15, 'Tài khoản hoạt động > 1 năm', 1),
('quick_response', 3, 'Phản hồi nhanh host/support', 1),
('complete_profile', 5, 'Hoàn thiện profile đầy đủ', 1),

-- Negative actions
('cancel_booking', -5, 'Hủy booking', 1),
('late_cancel_booking', -10, 'Hủy booking muộn (< 24h)', 1),
('receive_1_star', -10, 'Nhận đánh giá 1 sao', 1),
('receive_2_star', -5, 'Nhận đánh giá 2 sao', 1),
('report_violation', -15, 'Bị báo cáo vi phạm', 1),
('admin_warning', -20, 'Cảnh báo từ admin', 1),
('fake_listing', -30, 'Đăng listing giả mạo', 1),
('spam_message', -25, 'Spam tin nhắn', 1),
('payment_dispute', -15, 'Tranh chấp thanh toán', 1),
('severe_violation', -50, 'Vi phạm nghiêm trọng', 1),
('support_complaint', -10, 'Khiếu nại từ support', 1);
