...
// Kiểm tra đã đăng nhập chưa - nếu rồi thì redirect
if (isset($_SESSION['user_id'])) {
  $mUser = new mUser();
  $user = $mUser->mGetUserById($_SESSION['user_id']);
...
if (isset($_GET['auto_send']) && $_GET['auto_send'] == '1') {
  $mUser = new mUser();
  $newCode = $mUser->mGenerateVerifyCode($userId);
  ...
  if ($newCode) {
    ...
    if ($emailSent) {
      ...
      $errorMessage = 'Không thể gửi mã. Vui lòng thử lại sau.';
    }
  } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_code'])) {
  ...
  if (empty($code)) {
    ...
    $errorMessage = 'Mã xác thực phải là 6 chữ số';
  } else {
    $mUser = new mUser();
...
<link rel="stylesheet" href="../css/verify-code.css?v=<?php echo time(); ?>">
